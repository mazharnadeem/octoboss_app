...
class _ReviewScreenState extends State<ReviewScreen> {
  ...
  addReview(var octoId,var rating,var review) async{
    try{
      var data={
      ...
      if(response.statusCode==201){
        ...
        print('Review added : 201');
        var data2=jsonDecode(response.body.toString());
        deleteoffer(offer_data['id']);
        updateIssueStatus(offerdata['issue_id'], 'done');
        updatePublicStatus(offerdata['issue_id'],'private');
        updateChatStatus(offerdata['id'], 'completed');
        octobossJobDone(
            offerdata['sender_id'],
            offerdata['receiver_id'],
            offerdata['issue_id'],
            offerdata['location'].toString(),
            offerdata['description'].toString(),
            offerdata['problem'].toString(),
            offerdata['language'].toString(),
            offerdata['price'],
            offerdata['time'],
            offerdata['dates'],
            offerdata['duration'],
        ...
        print('Review added : 200');
        var data2=jsonDecode(response.body.toString());
        return false;
      }
      else{
        print('Review added : else');
        var data2=jsonDecode(response.body.toString());
        return false;
      }
  ...
  octobossJobDone(var senderId ,var receiverId, var issueId, var location, var description, var problem, var languages, var price, var time, var date, var duration, var rating ,var image) async{
    try{
      ...
      if(response.statusCode==200){
        print('Job done : 200');
        var data2=jsonDecode(response.body.toString());
        Fluttertoast.showToast(msg: '${data2}');
        return false;
      }
      else{
        print('Job done Failed : else');
      }
